...
//
// Compile (Skylake+):
//   g++ -O3 -std=c++17 -march=skylake-avx512 -fopenmp spgemm_avx512.cpp -o spgemm
// --------------------------------------------------
...
CSC coo_to_csc(int nrows, int ncols, const std::vector<COO>& coo) {
    ...
    for (auto& e : coo) A.col_ptr[e.j + 1]++;
    for (int c = 0; c < ncols; ++c) A.col_ptr[c + 1] += A.col_ptr[c];
...
// ------------------------
std::vector<BRowCompressed>
build_B_rows(int nrows, const std::vector<COO>& coo) {
...
// ------------------------
std::vector<BRowMasked>
build_B_rows_masked(int nrows, int ncols, const std::vector<COO>& coo) {
...
#ifndef NDEBUG
        int pc = 0;
        for (auto m : rows[r].masks) pc += _mm_popcnt_u32(m);
...
// PAD preparation
// ------------------------
std::vector<BRowPadded>
pad_B_rows(const std::vector<BRowCompressed>& B, int ncols) {
...
std::vector<val_t>
spgemm_pad_avx512(const CSC& A,
                  const std::vector<BRowPadded>& B,
                  int ncols)
{
    int padded_cols = ((ncols + W - 1) / W) * W;
...
std::vector<val_t>
spgemm_expand_once_avx512(const CSC& A,
                          const std::vector<BRowCompressed>& B,
                          int ncols)
{
    int padded_cols = ((ncols + W - 1) / W) * W;
    std::vector<val_t> C(A.nrows * padded_cols, 0.f);
...
std::vector<val_t>
spgemm_expand_mask_avx512(const CSC& A,
...
int main(int argc, char** argv) {
	...
	auto t1 = now_ms();
	std::cout << Ar << "," << Ar << "," << (t1 - t0) << "," << file_name << "," << file_name <<  std::endl;
	...
	auto t1 = now_ms();
	std::cout << Ar << "," << Ar << "," << (t1 - t0) << "," << file_name << "," << file_name;
